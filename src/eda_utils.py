...
import pandas as pd
import librosa
from .audio_preprocessing import load_audio, preprocess_audio
...
def quick_audio_stats(path, sr=32000, max_seconds=30):
    try:
        ...
        return {
            ...
            "silence_ratio": float(np.mean(rms < np.percentile(rms, 20))) if len(rms) else np.nan,
        }
    except Exception as e:
        return {"error": str(e)}

def sample_per_class(df, label_col="label", n=2, random_state=42):
...
def group_by_barplot(df, col_str, count_str):
    ...
    plt.tight_layout()
    plt.savefig(f"/outputs/figures/{col_str}_valuecount.png", format='png', bbox_inches='tight')
    plt.show()
...
def show_skewness(df, df_str, col, xlabel, max_value=None):
    ...
    plt.tight_layout()
    plt.savefig(f"/outputs/figures/{df_str}_{col}_skewness.png", format='png', bbox_inches='tight')
    plt.show()
...
def waveshow(path, data_str):
    ...
    plt.tight_layout()
    plt.savefig(f"/outputs/figures/{data_str}_waveshow.png", format='png', bbox_inches='tight')
    plt.show()
...
def specshow(path, data_str):
    ...
    plt.tight_layout()
    plt.savefig(f"/outputs/figures/{data_str}_spectshow.png", format='png', bbox_inches='tight')
    plt.show()
...
def specshow_mfcc(path, data_str):
    ...
    plt.tight_layout()
    plt.savefig(f"/outputs/figures/{data_str}_spectshow_mfcc.png", format='png', bbox_inches='tight')
    plt.show()
...
def compare_mfcc(df, col, col_str, family, min_n=20):
    ...
    plt.tight_layout()
    if col == "common_name":
        plt.savefig(f"/outputs/figures/{col}_{family}_MFCC.png", format='png', bbox_inches='tight')
    else:
        plt.savefig(f"/outputs/figures/{col}_MFCC.png", format='png', bbox_inches='tight')
    plt.show()
